...
- `LLMConfig.max_retries` (default 4) wired through to the Anthropic SDK's
  built-in exponential backoff.
- Optional JSONL run logging via `RunLogger`. Off by default; pass
  `LLMClient(logger=RunLogger("run.jsonl"))` to capture one line per call.
...
### Changed
- Critic and Comparator now use Anthropic tool-use with a JSON schema instead
  of last-line text parsing. Eliminates the silent ACCEPT-on-malformed-output
